...
function selectFunction(functionType) {
    ...
    // 如果选择的不是理工速知，显示暂未推行
    if (functionType !== 'science') {
        showToast('此功能暂未推行，敬请期待');
    }
}
...
function updateAnalyzeButton() {
    ...
    const hasValidFunction = selectedFunction === 'science';
...
async function analyzeDocument() {
    if (!currentFile || selectedFunction !== 'science' || isAnalyzing) {
        return;
    }
    ...
    try {
        ...
        // 加载系统提示词
        const systemPrompt = await loadSystemPrompt('science');
...
// 加载系统提示词
async function loadSystemPrompt(functionType) {
    try {
        const response = await fetch(`prompts/${functionType}.json`);
        if (!response.ok) {
        ...
        // 返回默认的理工速知提示词
        return `你是一个专业的理工科课件整理助手。请分析用户提供的文档内容，并按照以下要求整理知识点：
...
function reRenderMath() {
    ...
    console.log('开始手动重新渲染数学公式...');
    showToast('正在重新渲染数学公式...');
    
    // 从原始markdown重新渲染
    if (window.currentMarkdown) {
        try {
            renderContentDirectly(window.currentMarkdown, resultContent);
            console.log('重新渲染完成');
        } catch (error) {
            console.error('重新渲染失败:', error);
            showToast('重新渲染失败：' + error.message);
        }
    } else {
        showToast('没有原始内容可用于重新渲染');
    }
}
