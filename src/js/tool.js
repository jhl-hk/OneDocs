...
function processFile(file) {
  ...
  switch (file.type) {
    ...
    case "application/vnd.openxmlformats-officedocument.presentationml.presentation":
      fileTypeHint = "已选择PowerPoint文档，正在准备解析...";
      break;
    case "text/plain":
...
function updateAnalyzeButton() {
  ...
  const hasValidFunction = ["science", "liberal", "data", "news"].includes(
    selectedFunction,
  );
...
async function analyzeDocument() {
  if (
    !currentFile ||
    !["science", "liberal", "data", "news"].includes(selectedFunction) ||
    isAnalyzing
  ) {
    return;
  }
  ...
  if (!apiKey) {
    const providerName =
      MODEL_PROVIDERS[currentProvider]?.name || currentProvider;
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = function (e) {
      ...
      if (file.type === "text/plain") {
        ...
        // PowerPoint文档处理需要JSZip库
        extractPowerPointText(content).then(resolve).catch(reject);
      } else {
...
// 提取PowerPoint文本 (使用JSZip)
async function extractPowerPointText(arrayBuffer) {
  try {
    // 检查JSZip是否可用
    if (typeof JSZip === "undefined") {
      throw new Error("PowerPoint文档处理库未加载，请刷新页面重试");
    }
...
async function callAI(systemPrompt, content, provider, apiKey) {
  ...
  const model =
    localStorage.getItem(`${provider}_model`) || config.defaultModel;
  const baseUrl =
    localStorage.getItem(`${provider}_base_url`) || config.baseUrl;
...
function renderMathWithKaTeX(targetElement) {
  ...
  try {
    ...
    console.log(
      `发现 ${blockFormulas} 个块级公式，${inlineFormulas} 个行内公式`,
    );
...
function exportToPdf() {
  ...
  console.log(
    "要导出的内容:",
    resultContent.innerHTML.substring(0, 200) + "...",
  );
  showToast("正在准备导出...");
...
function loadProviderSettings(provider) {
  ...
  const baseUrl =
    localStorage.getItem(`${provider}_base_url`) ||
...
async function testConnection() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      showToast(
        `❌ ${config.name} 连接失败 (${response.status})\n\n错误信息: ${errorMsg}`,
      );
    }
  } catch (error) {
